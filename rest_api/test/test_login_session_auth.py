...
class TestSessionAuthentication(APITestCase):
    ...
    def test_get_all_movies_protected_view(self):
        ...
        self.assertEqual(movie_response.status_code, status.HTTP_200_OK, 'error in get_all_movies API')

        self.assertEqual(self.login_response.status_code, status.HTTP_200_OK, 'error in login api')
        self.assertEqual(self.login_response.data, {'message':'Login successful'},'Message does not match')