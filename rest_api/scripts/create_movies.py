...
def run():
    ...
    for i in range(1, 5000):
        movie = Movies(
            name=faker.name(),
            lead_actor=faker.last_name(),
            director=faker.first_name(),
            producer=faker.first_name_nonbinary(),
            banner=faker.suffix(),
            distributor=faker.bs(),
            release_date=faker.date(),
            rating=float("{0:.1f}".format(random.uniform(4, 9.8))),
        )
