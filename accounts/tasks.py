...
def create_object_task():
    obj = Movies.objects.create(
        name=fake.name(), director=fake.job(), producer=fake.company()
    )
...
@shared_task(
    bind=True
)  # can self as an argument in function and can access task_name in function and do some math
...
def create_cron_job_celery_beat():
    ...
    PeriodicTask.objects.create(
        crontab=schedule,
        name=f"send_mail_at_3:26pm | {datetime.datetime.now().second}",
        task="accounts.tasks.send_login_mail",
        kwargs=json.dumps(email_args),
        enabled=True,
    )
